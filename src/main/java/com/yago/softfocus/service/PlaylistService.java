...
import com.yago.softfocus.exception.DadosTemperaturaException;
...
public class PlaylistService {
	...
	public DadosPlaylistTemperaturaDTO buscaPorCidade(String nomeCidade) {
		DadosPlaylistTemperaturaDTO retorno;
		try {
			retorno = buscarPlaylistPorTemperatura(
			...
			throw new DadosTemperaturaException("Cidade não encontrada!");
		}
		return retorno;
	}
	...
	public DadosPlaylistTemperaturaDTO buscaPorCoordenadas(String latitude, String longitude) {
		DadosPlaylistTemperaturaDTO retorno;
		try {
			retorno = buscarPlaylistPorTemperatura(restTemplate
					.getForObject("https://api.openweathermap.org/data/2.5/weather?lat=" + latitude + "&lon="
	...
	private DadosPlaylistTemperaturaDTO buscarPlaylistPorTemperatura(WeatherData dadosTemperatura) {
